...
public class GraphIsomorphism {
	...
	if (l2 != null){
     while(l != null){
  	   if(l2.equals(l)){
  		   pwg.println("0.2"); //that is the weight we assign for structural isomorphism and 0.2 is assigned when all are same.
  	   l=br.readLine();
     }
  	...
  	//if the two aren't equal then it is checked word by word
  	   ...
  	   String s1_words[] = l.split("\\s");
  	    ...
  	    if(i>=2)
  	    	pwg.println("0.2"); // 2 or greater than two instances are same
  	    else
