...
public class LuceneTester {
	...
	// this class executes the lucene approach
	...
	public void lucene_recommender() throws OWLOntologyCreationException, OWLOntologyStorageException, IOException {
	   ...
	   private void search(String searchQuery) throws IOException, ParseException {
		 ...
		 PrintStream no=new PrintStream(System.getProperty("user.dir")+"/resources/lucene_scores"); 
	      ...
	      for(ScoreDoc scoreDoc : hits.scoreDocs) {
	         Document doc = searcher.getDocument(scoreDoc);
	                        ...
	                        no.println(scoreDoc.score);
	         // preco_lucene.println(doc.get(LuceneConstants.FILE_NAME)+ " with score " + scoreDoc.score); 
	           lines++;
	              
	      } 
	      
	     NormalisingLuceneScore n=new NormalisingLuceneScore();
	      n.normalisingScores(lines);
	      ...
	      //mapping with IRIs so that IRI is also present alongwith ODP name
			...
			for(int kl=0;kl<lines.size();kl++){
			   BufferedReader bodp=new BufferedReader(new FileReader(System.getProperty("user.dir")+ "/resources/odp_list_for_lucene"));
