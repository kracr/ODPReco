...
/*<-----------------------------------------BEHAVIOURAL ANALYSIS-------------------------------------------->
	...
	/*--------------------------------EXECUTION PART------------------------------------------------
     ...
     // The structure of the ontology is analysed by using isomorphism concept
               
              GraphConstruction cont=new GraphConstruction();
              cont.graphcons();
	 ...
	 */
	   ODPRecommender reco=new ODPRecommender();
	    reco.odpRecommender();
	  /*-----------------------------------------LUCENE RECOMMENDER---------------------------------------->
	  * Lucens is the other approach we have followed for recommendations. It creates index file and matches 
	  * the recommends the top 5 documents which are semantically similar
	  */
	    LuceneTester lt=new LuceneTester();
	    lt.lucene_recommender();
	   /*--------------------------------------COMBINING THE TWO SCORES------------------------------------->
	    * Integrating the two scores obtained via doc2veca approach and lucene and printing these
	    */
	    
	    //merging the two recommendation files
	    PrintWriter pwf = new PrintWriter(System.getProperty("user.dir")+ "/resources/integrated_reco"); 	          
        // BufferedReader to obtain the signature files of the ODP and the ontology
        BufferedReader br1f = new BufferedReader(new FileReader(System.getProperty("user.dir")+ "/resources/recommender")); 
        BufferedReader br2f = new BufferedReader(new FileReader(System.getProperty("user.dir")+ "/resources/final_lucenereco")); 	          	          
        String line1f = br1f.readLine(); 
        String line2f = br2f.readLine(); 	          
        // loop is used to copy the two files into the merged one	        
        while (line1f != null || line2f !=null) 
        { 
            if(line1f != null) 
            { 
                pwf.println(line1f); 
                line1f = br1f.readLine(); 
            } 
              
            if(line2f != null) 
            { 
                pwf.println(line2f); 
                line2f = br2f.readLine(); 
            } 
        } 	      
        pwf.flush(); 	          
       ...
       //sorting the file 
        ...
        BufferedReader br_final=new BufferedReader(new FileReader(System.getProperty("user.dir")+ "/resources/integrated_reco"));

   	    List<String> lines_final = new ArrayList<String>();
        String line_f = null;
         
        while ((line_f = br_final.readLine()) != null)
        {
            lines_final.add(line_f);
        }
        ...
        for (String line_read : lines_final) 
        {
       	
            System.out.println(line_read);
        }
   Collections.sort(lines_final,Collections.reverseOrder());
   PrintStream recofin=new PrintStream(System.getProperty("user.dir")+ "/resources/final_reco"); 	          
  
 
   Set<String> set_reco = new LinkedHashSet<String>(lines_final);    
   Iterator<String> itr = set_reco.iterator();

   while (itr.hasNext()) {

       recofin.println(itr.next());
   }
   
   recofin.close();
   
   BufferedReader brodpreco=new BufferedReader(new FileReader(System.getProperty("user.dir")+"/resources/final_reco"));
   PrintWriter reco_final = new PrintWriter(System.getProperty("user.dir")+ "/resources/integrated_reco"); 	          
   String line_reco=brodpreco.readLine();
	String sreco="";

	 while(line_reco!= null) 
     { 
         sreco=sreco + line_reco; 
         sreco=sreco.replaceAll("\\d","");
         sreco=sreco.replaceFirst(".","");
         reco_final.println(sreco);
         line_reco = brodpreco.readLine();
         sreco="";
	 ...
	 //hash set is used so that if the ODP is same from the two approaches, then
	 ...
	 BufferedReader bfinc=new BufferedReader(new FileReader(System.getProperty("user.dir")+"/resources/integrated_reco"));
	 List<String> lines_finalc = new ArrayList<String>();
     String line_fc = null;
     
     while ((line_fc = bfinc.readLine()) != null)
     {
         lines_finalc.add(line_fc);
     }
      
     bfinc.close();
     Set<String> set_recof = new LinkedHashSet<String>(lines_finalc); 
     Iterator<String> itrc = set_recof.iterator();
